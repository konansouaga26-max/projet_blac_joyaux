<header class="sticky top-0 z-40 bg-bj-noir text-bj-creme shadow-md">
    <nav class="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <a href="{{ url('/') }}" class="flex items-center gap-2">
            ...
            <span class="font-titre text-xl tracking-widest uppercase">Blac <span
                    class="text-bj-or">Joyaux</span></span>
        </a>
        <ul class="hidden md:flex items-center gap-8 text-sm font-medium uppercase tracking-wide">
            ...
            <li><a href="{{ route('admin.dashboard') }}"
                    class="text-bj-or hover:text-bj-creme transition-colors">Admin</a></li>
            @endif
            @endauth
        </ul>
        <div class="flex items-center gap-4">
            ...
            <a href="{{ url('/panier') }}" class="relative hover:text-bj-or transition-colors" aria-label="Mon panier">
                ...
                <span id="badge-panier"
                    class="absolute -top-2 -right-2 bg-bj-or text-bj-noir text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center">{{
    ...
    <div id="menu-mobile" class="hidden md:hidden bg-bj-noir border-t border-white/10">
        <ul class="flex flex-col px-4 py-3 gap-1 text-sm uppercase tracking-wide">
            ...
            <li><a href="{{ url('/essayage') }}" class="block py-2.5 px-2 rounded hover:bg-white/10">Essayage
                    virtuel</a></li>
            <li><button data-modal-target="modal-histoire" data-modal-toggle="modal-histoire"
                    class="block w-full text-left py-2.5 px-2 rounded uppercase hover:bg-white/10">Notre
                    Histoire</button></li>
            <li><a href="{{ url('/connexion') }}"
                    class="block py-2.5 px-2 rounded text-bj-or hover:bg-white/10">Connexion</a></li>
        </ul>
    </div>
</header>
